...
<template>
  <div>
    <v-row>
      <v-col cols="12">
        <h2 class="text-h5 my-4">{{ title }}</h2>
      ...
      <v-col cols="12">
        <v-container class="max-width">
          <v-pagination v-if="page_cnt > 0" v-model="page" :length="page_cnt" rounded="circle"
                        @update:modelValue="change_page"></v-pagination>
        </v-container>
      </v-col>
    </v-row>
  </div>
...
const { data } = await useAsyncData(`list-${route.fullPath}`, async () => {
