...
async function mount(page) {
    ...
    await page.addStyleTag({ path: resolve(process.cwd(), 'public/static/js/ai-top5.css') });
    await page.addScriptTag({ path: resolve(process.cwd(), 'public/static/js/ai-top5.js') });
...
test('renders the ordered light-theme TOP5 panel without executing raw HTML', async ({ page }) => {
    ...
    await expect(page.locator('.summary-duck')).toHaveScreenshot('ai-top5-light.png', { animations: 'disabled' });
});
...
test('renders high-contrast dark tokens and keyboard focus', async ({ page }) => {
    ...
    await expect(page.locator('.summary-duck')).toHaveScreenshot('ai-top5-dark.png', { animations: 'disabled' });
});
