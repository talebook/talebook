...
const nodes = Array.from({ length: 36 }, (_, index) => ({
    ...
    mentions: 3,
    citations: [{ href: 'OPS/chapter2.html', start: 8, end: 24, quote: '张英才背着行李出门时' }],
}));
const relations = Array.from({ length: 35 }, (_, index) => ({
    ...
    citations: [{ href: 'OPS/chapter2.html', start: 8, end: 24, quote: '张英才背着行李出门时' }], mentions: 1,
}));
const artifact = {
    id: '33333333-3333-3333-3333-333333333333', feature: 'knowledge_graph', book_id: 1,
    book_version: 'fixture', scope: { kind: 'book', label: '全书', chapter_hrefs: ['OPS/chapter2.html'], chapter_count: 33, character_count: 204279 },
...
declare global {
    interface Window {
        ...
        TalebookKnowledgeGraph: { open: () => void };
    }
}
