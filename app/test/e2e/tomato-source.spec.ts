...
test('Tomato source configuration submits an edited timeout as a number', async ({ page, request }, testInfo) => {
    ...
    await page.route('**/api/admin/plugins', async route => {
        ...
        Object.assign(plugin, {
            ...
            name: '番茄小说下载器',
            description: '搜索番茄小说或输入书籍 ID，下载完整 TXT 到本地书库。需安装下载器，依赖上游在线服务。',
            config_schema: {
    ...
    expect((await submitted).postDataJSON().config.timeout_seconds).toBe(900);
    await page.setViewportSize({ width: 375, height: 812 });
