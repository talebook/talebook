<template>
  <div id="txt-main">
    <v-navigation-drawer v-model="sidebar" location="start" temporary width="240"
                         class="d-flex flex-column" style="height: 100%">
      <v-list-subheader style="height: 48px">{{ name }}</v-list-subheader>
      <v-virtual-scroll
        ...
        <template v-slot:default="{ item, index }">
          <v-list-item dense :key="item.title" @click="getNovelContent(index)"
                       :active="selected === index" color="primary">
              <v-list-item-title>
                {{ item.title }}
              </v-list-item-title>
          </v-list-item>
    ...
    <v-app-bar class="px-0" color="blue" density="compact" theme="dark">
      <v-app-bar-title class="ml-n5 mr-12 align-center d-flex">
        <v-app-bar-nav-icon @click.stop="sidebar = !sidebar">
          <v-icon>mdi-menu</v-icon>
        </v-app-bar-nav-icon>
        <span class="cursor-pointer ml-2" @click="router.push('/')">
          {{ name }}
        </span>
      </v-app-bar-title>
    </v-app-bar>
    ...
    <div style="margin-top: 64px;">
      <v-container>
        <v-card variant="outlined" v-if="!inited" width="300" style="margin: 0 auto">
        ...
        <div v-else>
          ...
          <div class="d-flex justify-space-between mt-4" v-show="novelContent && !loading">
            <v-btn color="info" elevation="0" :disabled="selected===0"
                   @click="getNovelContent(selected-1)">
              上一章
            </v-btn>
            <v-btn variant="outlined" elevation="0" @click="sidebar=true" v-show="!sidebar">
              目录
            </v-btn>
        ...
        <app-footer v-if="mainStore.nav"></app-footer>
      </v-container>
    </div>
  </div>
</template>
