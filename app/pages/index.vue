<template>
    ...
    <v-row>
        ...
        <v-col cols=12>
            <BookCards :books="get_recent_books"></BookCards>
        </v-col>
    </v-row>
    <v-row>
        <v-col cols=12>
        ...
        <v-col cols=12 sm=6 md=4 v-for="nav in navs" :key="nav.text">
            <v-card outlined>
                <v-list>
                    <v-list-item :to="nav.href" :title="nav.text" :subtitle="nav.subtitle">
...
const { data: indexData } = await useAsyncData('index', async () => {
