  ...
  rest: {
    repos: {
      getCollaboratorPermissionLevel: async (args) => {
        ...
        if (process.env.TEST_PERMISSION === "lookup-error") throw new Error("permission lookup failed");
...
def test_issue_events_cannot_trigger_codex_even_with_template_labels():
    triggers = workflow_data()[True]
    job_condition = codex_job()["if"]

    assert "issues" not in triggers
    assert "github.event_name == 'issues'" not in job_condition
    assert "github.event.issue.labels" not in job_condition
...
@pytest.mark.parametrize(
    ("actor", "labels"),
    (
        ("external-reporter", [{"name": "bug"}]),
        ("external-reporter", [{"name": "need help"}]),
        ("maintainer", [{"name": "bug"}]),
        ("automation[bot]", [{"name": "bug"}]),
    ),
)
@pytest.mark.parametrize("action", ["opened", "labeled", "reopened"])
@requires_node
def test_all_issue_contexts_fail_closed(tmp_path, actor, labels, action):
    payload = {
        "action": action,
        "issue": {
            "number": 929,
    ...
    assert trace["request"]["requestKind"] == "issues"
...
@requires_node
def test_manual_issue_comment_context_still_executes_for_a_repository_writer(tmp_path, permission):
    payload = {
        "action": "created",
        "comment": {
            "id": 123,
    ...
    assert trace["request"]["actorPermission"] == permission
    assert trace["request"]["authorizationKind"] == "repository_writer"
    assert "designReviewPreapproved" not in trace["request"]
    call_names = [call["name"] for call in trace["calls"]]
...
@pytest.mark.parametrize("permission", ["none", "read", "triage", "lookup-error"])
@requires_node
def test_manual_request_without_write_permission_fails_closed(tmp_path, permission):
    payload = {
        "action": "created",
        "comment": {
            "id": 123,
        ...
        "issue": {"number": 875, "title": "bug", "labels": [{"name": "bug"}]},
        "repository": {"default_branch": "master"},
    }
    completed, trace = run_context_step(
        tmp_path,
    ...
    call_names = [call["name"] for call in trace["calls"]]
    assert "getCollaboratorPermissionLevel" in call_names
    assert not {"createComment", "getContent", "getBranch", "createForIssueComment"}.intersection(call_names)
...
def test_agent_prompt_requires_explicit_maintainer_requests():
    prompt = prompt_text()

    assert "automatic_bug_issue" not in prompt
    assert "预授权" not in prompt
    assert "Issue 标签不代表授权" in prompt
    assert "不受信任的输入" in prompt
    assert "Draft PR 必须人工评审" in prompt
