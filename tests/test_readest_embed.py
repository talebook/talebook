...
class TestReadestEmbed(TestWithUserLogin):
    def test_readest_entry_keeps_book_read_permission_gate(self):
        ...
        self.assertEqual(location.path, "/readest/talebook-launch.html")
        self.assertEqual(query["bookId"], [str(BID_EPUB)])
        self.assertEqual(set(query), {"bookId"})
    ...
    def test_embed_static_deployment_contract_has_csp_and_cache_boundaries(self):
        ...
        launcher = (readest_root / "talebook-launch.html").read_text(encoding="utf-8")
        launcher_script = (readest_root / "talebook-launch.js").read_text(encoding="utf-8")
        source = json.loads((readest_root / "TALEBOOK_SOURCE.json").read_text(encoding="utf-8"))
        ...
        read_proxy = (root / "app" / "server" / "routes" / "read" / "[...path].ts").read_text(encoding="utf-8")
        ...
        for nginx in nginx_configs:
            ...
            self.assertIn("location = /readest/talebook-launch.html", nginx)
            self.assertIn("location = /readest/legacy-worker-cleanup.js", nginx)
            self.assertIn("location ^~ /read/resource/", nginx)
            self.assertIn("location ^~ /_nuxt/", nginx)
            self.assertIn("try_files $uri =404", nginx)
            self.assertNotIn("stale-nuxt-recovery", nginx)
            self.assertIn("Content-Security-Policy", nginx)
            self.assertIn("script-src 'self' 'wasm-unsafe-eval'", nginx)
            self.assertIn("connect-src 'self' data:", nginx)
            self.assertIn("worker-src 'self' blob:", nginx)
            self.assertIn('Cross-Origin-Opener-Policy "same-origin"', nginx)
            self.assertIn('Cross-Origin-Embedder-Policy "require-corp"', nginx)
            self.assertIn("immutable", nginx)
            self.assertIn('add_header Service-Worker-Allowed "/" always', nginx)
        self.assertIn("/readest/**", nuxt)
        self.assertIn("'Service-Worker-Allowed': basePath + '/'", nuxt)
        self.assertIn("'/readest/legacy-worker-cleanup.js'", nuxt)
        self.assertIn("'/readest/talebook-launch.html'", nuxt)
        self.assertIn("script-src 'self' 'wasm-unsafe-eval'", nuxt)
        self.assertIn("connect-src 'self' data:", nuxt)
        self.assertIn("worker-src 'self' blob:", nuxt)
        self.assertIn("'Cross-Origin-Opener-Policy': 'same-origin'", nuxt)
        self.assertIn("'Cross-Origin-Embedder-Policy': 'require-corp'", nuxt)
        self.assertNotIn("'/read/**': { proxy:", nuxt)
        self.assertNotIn("stale-nuxt-recovery", nuxt)
        self.assertNotIn("errorHandler:", nuxt)
        self.assertIn("redirect: 'manual'", read_proxy)
        self.assertIn("buildReadProxyResponse", read_proxy)
        self.assertIn('src="./talebook-launch.js"', launcher)
        self.assertIn("reader-bootstrap?engine=readest", launcher_script)
        self.assertIn("application/epub+zip", launcher_script)
        self.assertIn("mokeBookId", launcher_script)
        self.assertIn("mokeSourceServerUrl", launcher_script)
        self.assertEqual(source["repository"], "https://github.com/hehetoshang/readest-reader")
        self.assertEqual(source["commit"], "a4fc84077ad4f4c490305961235086f25bd4da12")
        self.assertEqual(source["readerAppVersion"], "0.12.1")
        self.assertEqual(source["build"]["appPlatform"], "web")
        self.assertTrue((readest_root / "LICENSE").exists())
        self.assertTrue((readest_root / "THIRD_PARTY_NOTICES.md").exists())
        ...
        self.assertIn("forceCacheCleanup: true", cleanup_script)
        self.assertNotIn("recoverStaleNuxtPage", cleanup_script)
        ...
        self.assertFalse((readest_root / "stale-nuxt-recovery.js").exists())
        self.assertFalse((root / "app" / "server" / "error-handler.ts").exists())
        ...
        reader_bundles = list((service_worker.parent / "_next" / "static" / "chunks" / "pages").glob("reader-*.js"))
        self.assertEqual(len(reader_bundles), 1)
        reader_bundle = reader_bundles[0].read_text(encoding="utf-8")
        self.assertIn("mokeSourceServerUrl", reader_bundle)
        self.assertIn("online.range_unsupported", reader_bundle)
