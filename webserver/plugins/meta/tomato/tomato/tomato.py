...
class Page(object):
    ...
    def get_image(self):
        ...
        # 3. 从 HTML 中提取封面 URL
        cover_url = (
            regex_json_string_field(self.html, "thumb_url")
            or regex_json_string_field(self.html, "expand_thumb_url")
            or regex_json_string_field(self.html, "cover_url")
            or regex_json_string_field(self.html, "cover")
        )
        if not cover_url:
