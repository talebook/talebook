...
class XhsdBookApi:
    ...
    def _get_detail_url_by_isbn(self, isbn):
        ...
        try:
            ...
            for item in soup.find_all("li", class_="product"):
                ...
                detail_url = (
                    "https:" + relative_url if relative_url.startswith("//") else urljoin(self.base_detail_url, relative_url)
                )
    ...
    def _metadata(self, info):
        ...
        mi.authors = [author] if author else ["佚名"]
        ...
        mi.source = "新华书店"
        mi.provider_key = KEY
        mi.provider_value = info.get(
            "isbn"
        )  # Use ISBN as provider value since we don't have a unique ID easily accessible or it's the same
