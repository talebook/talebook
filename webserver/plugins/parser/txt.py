...
class TxtParser:
    ...
    TXT_CONTENT_RULES = [
        ...
        {"name": "正文 标题/序号", "example": "正文 我奶常山赵子龙", "rule": r"^[ 　\t]{0,4}正文[ 　]{1,4}.{0,20}$"},
        {
    ...
    def __init__(self):
        ...
        for rule_info in self.TXT_CONTENT_RULES:
            try:
                ...
                self.compiled_rules.append({"name": rule_info["name"], "example": rule_info["example"], "rule": compiled_rule})
    ...
    def parse_txt_book_toc(self, fileobj):
        ...
        while line:
            ...
            for rule_info in self.compiled_rules:
                ...
                pre_chapter = {"id": idx, "title": matches[0].strip(), "start": seek_position, "end": -1}
            ...
            line = fileobj.readline()
        if len(toc) == 0:
            toc = [{"id": 1, "title": "全部", "start": 0, "end": -1}]
        return toc
