    ...
    def post(self):
        ...
        try:
            ...
            if not url.startswith(("http://", "https://")):
                return {"err": "params.url.invalid", "msg": _("OPDS URL 必须以 http:// 或 https:// 开头")}
    ...
    def put(self):
        ...
        try:
            ...
            if "name" in req:
                ...
                # 检查名称是否与其他配置冲突
                existing = self.session.query(OpdsSource).filter(
                    OpdsSource.name == name,
                    OpdsSource.id != source_id
                ).first()
                if existing:
                    return {"err": "params.name.exist", "msg": _("OPDS 源名称已存在")}
                source.name = name
