...
运行 `make check-spec` 校验：章节结构与顺序、一级标题与文件名一致、定义一节含「不是什么」边界、术语表四列且示例非空、实现对照子表为五者子集且保序、内部互链有效且链接文案等于目标文件名、索引收录每一篇、实现对照引用的仓库路径真实存在、**7.2 子表中每行的完整接口路径**命中真实注册路由（同行以 `/run`、`/status` 形式省略续写的不计），以及已注册内置插件在 `spec/插件/` 下有归属。
