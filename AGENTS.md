...
`spec/` 是 Talebook 的领域规格文档，描述产品**是什么**、按什么规则运转。它与 `design/` 的分工是硬边界：

- **`spec/` 回答「是什么」**：当前的产品应然，唯一一份，总是最新，无时态。
- **`design/` 回答「怎么做」**：某次改动的论证与验证记录，有时态，带 WIP/ACTIVE/SUPERSEDED 生命周期。

规格文档的约束：

- 领域定义只写在 `spec/` ，不在别处重复。
- **不写未实现、不写已废弃、不写变更历史**。设计已定但代码未落地的功能不进入 `spec/` ，它留在对应的 `design/` 方案里。
- 目录与文件名使用中文；**协议、格式与外部产品的专名保留原写法**（OPDS、WebDAV、BRS、TXT、Legado 等）。一级标题必须与文件名一致，**索引篇除外**。
- 每篇固定七节，顺序为：定义（含「不是什么」边界）、使用场景、功能、术语与界面用词、行为逻辑、关联实体、实现对照。
- 术语表四列：标准用词、英文、含义、示例；示例必须是具体的书、作者或读者动作。
- 正文（第 1–6 节）解释产品行为与规则，第 7 节回答「在代码里的什么位置」。**不要在正文里写概念到代码位置的映射**（例如「书架即 `ReadingState.wants` 字段」「按 `bookSourceUrl` 去重」），这类映射属于第 7 节。正文可以自由使用领域词汇，包括英文术语、协议名与产品自身定义的稳定标识（插件 ID、能力名、用户可选的枚举值）。
- 第 7 节拆为数据存储、API 接口、关键定义、代码落点、界面文案五张子表，**按需缺省**：不适用时整张删除，保留的子表必须保持上述相对顺序。每张子表只写自己那一列的东西——「关键定义」说明符号是什么，指向文件是「代码落点」的职责。
- 文件名与文中主术语以领域术语为准，界面用词只写在第 4 节。
- `spec/索引.md` 是唯一不遵守七节骨架的文件：它必须存在、必须收录每一篇，且指向 `spec/` 内文件的链接文案必须等于目标文件名。

**改变产品行为的改动，必须在同一个 PR 内更新 `spec/` 对应篇目**；内部方案的「影响范围」需注明涉及的 `spec/` 文件，没有影响时说明原因。新增内置插件时必须在 `spec/插件/` 下给它归属：有专属产品界面的单独成篇，没有的写进对应能力类别的合集篇。

运行 `make check-spec` 校验：章节结构与顺序、一级标题与文件名一致、定义一节含「不是什么」边界、术语表四列且示例非空、实现对照子表为五者子集且保序、内部互链有效且链接文案等于目标文件名、索引收录每一篇、**仓库内其他文档指向 `spec/` 的链接有效**、实现对照引用的仓库路径真实存在、**7.2 子表中每行的完整接口路径**命中真实注册路由（同行以 `/run`、`/status` 形式省略续写的不计），以及已注册内置插件在 `spec/插件/` 下有归属。
...
make check-spec    # 领域规格：结构、术语表、互链、实现对照与插件覆盖度必须通过
...
- `spec/` 目录存放领域规格文档，描述产品当前的应然；唯一一份、总是最新、无时态。
