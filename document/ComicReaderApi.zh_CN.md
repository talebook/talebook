...
- `/read-comic/:bookId` 是 Tornado 路由，由后端先完成登录、权限、书籍媒体类型和容器选择校验，再渲染轻量 HTML host；Nginx 的 dev、SPA、SSR 配置及 Nuxt 本地开发代理均将该路径直接转发给 Tornado，不能通过前端 `$backend` 添加 `/api` 前缀；
