...
当前提供搜索、书籍详情和完整 **TXT** 下载入库。没有在线逐章阅读、封面代理、分页搜索、EPUB 原生导出、段评或有声书功能。详情中的临时鉴权封面地址不会交给浏览器。相同书名及作者沿用已有入库去重规则。

下载器返回“完成”仍会核对本次下载历史：成功章数必须等于选择章数且失败为零，否则拒绝入库。空文件、多个输出、符号链接或超过 64 MiB 的文件也会拒绝入库。作品版权不因下载器使用 MIT 协议而改变。
...
无须提供 Cookie、API 密钥或自建远程 API。**这不代表程序没有在线服务依赖**，见下节。每次操作会启动随机端口、随机口令保护的 `127.0.0.1` 服务，进程及其配置、日志、缓存、下载临时文件在操作成功或失败后统一清理。TXT 进入 Calibre 前的导入临时文件也会清理。不启动持久后台服务，不需要开放端口。只读挂载同时阻止运行文件被覆盖；适配器还通过上游 `CARGO` 开发态判断禁用启动热更新，并在每次执行前重新校验哈希。
...
真实 Linux amd64 musl 发行版，在宿主机和 Talebook 测试容器中均完成搜索《西游记》（19 条结果）、读取书 ID `7274791759849196579` 的详情、下载完整 TXT（2,190,078 字节），通过本次下载历史的完整性检查。另运行真实联网测试，将该 TXT 经既有导入方法写入独立 Calibre 书库，校验书名与完整字节一致，并确认导入临时文件已清理。网络结果随上游变化；该记录不保证未来可用。

隔离测试覆盖插件契约、ID 校验、完整下载、缺章拒绝、错误脱敏、哈希失败不执行、进程回收、临时文件清理和下载预算。真实联网回归可显式运行：
...
前端配置表单数字类型、中英文标签和仅搜索书源的浏览入口经过回归。界面截图来自隔离 mock API 自动化环境，仅用于界面验证，不代表真实网络下载验收：
