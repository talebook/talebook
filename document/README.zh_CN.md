...
不需要提前创建演示账号：首次以 `DEMO_USERNAME`（默认 `demo`）、密码 `demodemo` 登录时会自动创建该账号。演示账号可以看到“管理员设置”页面，但内容是 `webserver/settings.py` 里的出厂默认参数，并非站点真实配置；点击保存会提示成功，但不会写入任何配置，仅用于演示交互流程。
